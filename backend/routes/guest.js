...
if (!fs.existsSync(tempUploadsDirFinal)) {
  ...
  console.log(`[Guest Routes] Created upload directory: ${tempUploadsDirFinal}`);
} else {
...
const fileFilter = (req, file, cb) => {
  const allowedMimeTypes = ['image/', 'video/', 'audio/', 'audio/flac', 'audio/x-flac'];
...
router.post('/upload', optionalAuth, upload.single('file'), async (req, res) => {
  try {
    ...
    if (!fs.existsSync(absoluteFilePath)) {
      console.error(`[Guest Upload] ERROR: File not found at: ${absoluteFilePath}`);
      console.error(`[Guest Upload] Original path from multer: ${filePath}`);
      console.error(`[Guest Upload] Temp uploads dir: ${tempUploadsDirFinal}`);
    ...
    console.log(`[Guest Upload] File exists: ${fs.existsSync(absoluteFilePath)}`);
    console.log(`[Guest Upload] TempId: ${tempId}`);
    
    // Determine file type
    let detectedFileType = 'audio';
    const ext = path.extname(filename).toLowerCase();
    if (['.wav', '.mp3', '.flac', '.ogg', '.m4a', '.aac', '.wma'].includes(ext)) {
  ...
  try {
    ...
    if (testFileName) {
      ...
      if (fs.existsSync(testAudioPath)) {
        ...
        console.log(`[Guest Analyze] ✓ Using test file directly: ${testFileName} -> ${actualFilePath}`);
        ...
        // Format result
        const formattedResult = aiService.formatResult(analysisResult, detectedFileType);
    ...
    if (tempId) {
      ...
      if (guestData && guestData.isUpload) {
        ...
        isUploadedFile = true;
        console.log(`[Guest Analyze] Found uploaded file via tempId: ${tempId}`);
        console.log(`[Guest Analyze] File path: ${actualFilePath}`);
        
        // Verify file exists
        if (!fs.existsSync(actualFilePath)) {
          console.error(`[Guest Analyze] ERROR: File not found at stored path: ${actualFilePath}`);
          console.error(`[Guest Analyze] Temp uploads dir: ${tempUploadsDirFinal}`);
          console.error(`[Guest Analyze] Checking if file exists in temp dir...`);
          ...
          if (fs.existsSync(altPath)) {
            console.log(`[Guest Analyze] Found file at alternative path: ${altPath}`);
        ...
        console.log(`[Guest Analyze] File exists: ${fs.existsSync(actualFilePath)}`);
      } else {
        console.log(`[Guest Analyze] TempId not found: ${tempId}`);
        console.log(`[Guest Analyze] Available tempIds: ${Array.from(guestAnalyses.keys()).slice(0, 5).join(', ')}...`);
        return res.status(404).json({
          status: 'error',
    ...
    if (actualFilePath) {
      ...
      if (isUploadedFile) {
      ...
      } else if (normalizedPath.startsWith('uploads/temp/') || normalizedPath.startsWith('uploads\\temp\\')) {
        // This is a relative path to an uploaded temp file
        // Resolve relative to project root
        const projectRoot = path.resolve(__dirname, '../..');
        actualFilePath = path.resolve(projectRoot, normalizedPath);
        console.log(`[Guest Analyze] Resolved uploaded temp file: ${actualFilePath}`);
        ...
        if (fs.existsSync(testAudioPath)) {
          ...
          console.log(`[Guest Analyze] ✓ Found in testAudio: ${actualFilePath}`);
        } else {
          ...
          if (fs.existsSync(resolvedPath)) {
            ...
            console.log(`[Guest Analyze] ✓ Resolved relative path: ${actualFilePath}`);
          } else {
            ...
            if (fs.existsSync(cwdPath)) {
              ...
              console.log(`[Guest Analyze] ✓ Resolved from CWD: ${actualFilePath}`);
            } else {
              actualFilePath = null;
              console.log(`[Guest Analyze] ✗ Could not resolve path: ${normalizedPath}`);
            }
          }
        }
      }
